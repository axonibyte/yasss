...
| **The per-identity signup cap is not race-safe** | With `allowMultiUserSignups` off, `AddVolunteerEndpoint` reads `event.countVolunteers(...)` and then commits with nothing holding the gap, so simultaneous anonymous signups from one address all read zero and all proceed — 2 to 5 of 16 get through in `e2e/concurrency`, scenario H. Structurally the same defect as the slot-capacity race, which *is* now fixed via `RSVP.claim`. Not fixed here because closing it means giving `Volunteer.commit()` — 155 lines, and the write path under every signup in the product — a transactional overload so the count and the insert share a connection. Severity is anti-abuse rather than overbooking: the cap is spam control, and a slot cap still limits the damage. Scenario H pins that it neither crashes nor locks everyone out, and prints the over-admission. |
| **Two activities with the same name collapse into one on read** | `Event.getActivities()` collects into a `TreeSet` ordered by `(priority, shortDescription)` compared case-insensitively, so an event created with both `Setup` and `setup` returns only one of them — the other is silently unreachable through the API and absent from the grid. Creation accepts both, so the loss is invisible to the organiser until they look. Not fixed because that ordering is what the grid renders from, and changing the collection changes display order; pinned by `e2e/text`. |
| **Grid slot tiles are 18px tall, under the 44px touch target** | WCAG 2.5.5 asks for 44×44 CSS pixels; the grid's slot buttons measure 18px high. It follows from the grid being a matrix that holds five columns at any width — it cannot reflow to fewer without losing its meaning — so raising the tiles is a layout decision about the product's central screen rather than a CSS tweak. Deliberately not asserted in `tests/e2e/compat.spec.js`: pinning 18px would enshrine it and asserting 44px would just fail. |
